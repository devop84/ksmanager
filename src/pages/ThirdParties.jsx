...
function ThirdParties({ refreshKey = 0, onAddThirdParty = () => {}, onEditThirdParty = () => {} }) {
  ...
  const handleDelete = async (thirdPartyId) => {
    if (!window.confirm('Are you sure you want to delete this third party?')) return
    try {
      setDeletingId(thirdPartyId)
      await sql`DELETE FROM third_parties WHERE id = ${thirdPartyId}`
      setThirdParties((prev) => prev.filter((tp) => tp.id !== thirdPartyId))
    } catch (err) {
            ...
            <div className="flex flex-col gap-4">
                        ...
                        <tr key={tp.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm text-gray-600">
                            <div className="flex items-center gap-3">
                              <button
                                onClick={() => onEditThirdParty(tp)}
                                className="text-gray-500 hover:text-indigo-600 transition-colors"
                                aria-label="Edit third party"
                              >
                                <svg
                                  className="w-5 h-5"
                                  fill="none"
              ...
              <div className="md:hidden space-y-3">
                {paginatedThirdParties.length === 0 ? (
                  ...
                  paginatedThirdParties.map((tp) => (
                    <div key={tp.id} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
