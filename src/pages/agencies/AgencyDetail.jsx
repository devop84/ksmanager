...
function AgencyDetail({ agencyId, onBack, onEdit, onDelete, user = null }) {
  ...
  const determineStatus = (order) => {
    ...
    // Then determine status based on appointment dates
    const now = new Date()
      ...
      <div className="rounded-xl bg-white p-4 sm:p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
          ...
          <div className="xl:col-span-1">
            <DetailInfoPanel
              title={t('agencyDetail.info.title')}
            ...
            >
              <dl className="space-y-4 text-sm">
                <div>
                  <dt className="text-xs font-medium text-gray-500 uppercase">{t('agencyDetail.info.id', 'ID')}</dt>
                  <dd className="mt-1 text-gray-900 font-mono">#{agency.id}</dd>
                </div>
                <div>
                  <dt className="text-xs font-medium text-gray-500 uppercase">{t('agencyDetail.info.name', 'Name')}</dt>
                  <dd className="mt-1 text-gray-900">{agency.name || '—'}</dd>
                </div>
                <div>
