          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="lg:col-span-1">
              <div className="rounded-xl border border-gray-200 bg-gray-50 p-5 shadow-sm sticky top-6 space-y-4">
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase">{t('productDetail.metadata.productId', 'Product ID')}</p>
                  <p className="mt-1 text-sm text-gray-900 font-mono">#{product.id}</p>
                </div>
                {product.sku && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 uppercase">{t('productDetail.metadata.sku', 'SKU')}</p>
                    <p className="mt-1 text-sm text-gray-900 font-mono">{product.sku}</p>
                  </div>
                )}
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase">{t('productDetail.metadata.created', 'Created')}</p>
                  <p className="mt-1 text-sm text-gray-900">{formatDateTime(product.created_at)}</p>
                </div>
                {product.updated_at && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 uppercase">{t('productDetail.metadata.updated', 'Last update')}</p>
                    <p className="mt-1 text-sm text-gray-900">{formatDateTime(product.updated_at)}</p>
                  </div>
                )}
              </div>
            </div>
