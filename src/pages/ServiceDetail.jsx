    ...
    const fetchPackages = async () => {
      try {
        ...
        const result = await sql`
          ...
          ORDER BY sp.duration_hours ASC NULLS LAST, sp.name ASC
        `
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                title={t('serviceDetail.actions.edit', 'Edit service')}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button
                ...
                title={t('serviceDetail.actions.delete', 'Delete service')}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="lg:col-span-1">
              <div className="rounded-xl border border-gray-200 bg-gray-50 p-5 shadow-sm sticky top-6 space-y-4">
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase">{t('serviceDetail.metadata.serviceId', 'Service ID')}</p>
                  <p className="mt-1 text-sm text-gray-900 font-mono">#{service.id}</p>
                </div>
                {service.category_name && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 uppercase">{t('serviceDetail.metadata.category', 'Category')}</p>
                    <p className="mt-1 text-sm text-gray-900">{service.category_name}</p>
                  </div>
                )}
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase">{t('serviceDetail.metadata.created', 'Created')}</p>
                  <p className="mt-1 text-sm text-gray-900">{formatDateTime(service.created_at)}</p>
                </div>
                {service.updated_at && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 uppercase">{t('serviceDetail.metadata.updated', 'Last update')}</p>
                    <p className="mt-1 text-sm text-gray-900">{formatDateTime(service.updated_at)}</p>
                  </div>
                )}
              </div>
            </div>
