...
function ServicePackageForm({ package: pkg, service, onCancel, onSaved }) {
  ...
  const handleSubmit = async (event) => {
    ...
    const durationValue = formData.duration_hours !== '' ? Number(formData.duration_hours) : null
      ...
      if (isEditing) {
        await sql`
          ...
          SET service_id = ${serviceIdValue},
              name = ${formData.name},
              duration_hours = ${durationValue || null},
              price = ${priceValue},
        ...
        await sql`
          INSERT INTO service_packages (service_id, name, duration_hours, price, currency, description, is_active)
          VALUES (${serviceIdValue}, ${formData.name}, ${durationValue || null}, ${priceValue}, ${formData.currency}, ${formData.description || null}, ${formData.is_active})
        `
      }
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700" htmlFor="duration_hours">
                {t('servicePackageForm.fields.duration', 'Duration (hours)')}
              </label>
              <input
                id="duration_hours"
                name="duration_hours"
